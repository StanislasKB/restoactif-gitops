                        ...
                        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                            <thead class="bg-gray-50 dark:bg-gray-700">
                                <tr>
                                    ...
                                    <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Profil</th>
                            ...
                            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                                ...
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-gray-200">@if( $user->profil()!=null){{ $user->profil()->name }}@endif</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-gray-200">@if( $user->profil()!=null){{ $user->profil()->address }}@endif</td>
                                    <td class="px-4 py-4 whitespace-nowrap text-end text-sm font-medium">
                                        <form action="{{ route('admin.change.status',['id'=>$user->id]) }}" method="POST">
                                            @csrf
                                            @if ($user->status==1)
                                            <button type="submit" class="bg-red-500 text-white px-2 py-2 rounded-lg">
                                                Désactiver
                                            </button>
                                            @else
                                            <button type="submit" class="bg-success text-white px-4 py-2 rounded-lg">
                                                Activer
                                            </button>
                                            @endif
